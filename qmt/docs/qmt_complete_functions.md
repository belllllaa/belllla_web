...
> 本节整理自 [QMT 价值因子获取函数介绍](https://blog.csdn.net/easyquant_qmt/article/details/155524376) 及迅投 QMT 社区文档，用于基本面因子选股与价值策略开发。

### get_financial_data - 获取财务数据（价值因子）

**原型**（以 xtdata 或 ContextInfo 调用视 QMT 版本而定）:
```python
get_financial_data(
    field_list,      # 字段列表，见下
    stock_list,      # 股票代码列表
    report_type,     # 取数口径：'announce_time' / 'report_time'
    start_time='',
    end_time=''
)
```

- **说明**: 获取个股财务数据与基本面因子，用于价值、质量、成长等因子选股。
- **参数**:
  - **field_list**: 字段列表，**必须使用「表名.字段名」格式**，否则易报错或取数异常。
  - **stock_list**: 合约代码列表（如 `['000001.SZ', '600519.SH']`）。
  - **report_type**: 取数口径，**与回测是否产生未来函数强相关**：
    - **`'announce_time'`**：按**公告日期**取数，回测时仅能使用已披露数据，**推荐用于实盘与回测**。
    - **`'report_time'`**：按**报告期**取数，若在 Q1 就使用 Q3 数据会构成未来函数，慎用于回测。
  - **start_time / end_time**: 时间范围，可选。
- **返回**: 与 field_list、stock_list 对应的财务数据（多为 DataFrame 或 dict，以实际 QMT 接口为准）。
- **数据更新**: 财报披露日当晚 22:00 左右自动更新；使用前需确保本地已下载对应财务数据。

**常用财务表与字段示例**:

| 表名 | 说明 | 字段示例 |
|------|------|----------|
| CAPITALSTRUCTURE | 股本结构 | total_capital（总股本） |
| ASHAREINCOME | 利润表 | net_profit_incl_min_int_inc（归母净利润） |
| ASHARECASHFLOW | 现金流量表 | net_cash_flows_oper_act（经营活动现金流净额） |
| Balance | 资产负债表 | 总资产、总负债等 |
| Income | 利润表 | 营收、净利润等 |
| CashFlow | 现金流量表 | 经营/投资/筹资现金流 |
| Capital | 股本表 | 股本变动 |
| PERSHAREINDEX | 每股指标 | 每股收益、每股净资产等（部分字段存在返回 0 的情况，需做缺失或异常处理） |
| HolderNum / Top10Holder / Top10FlowHolder | 股东与持股 | 股东人数、前十大股东等 |

**示例**:
```python
# 取总股本、归母净利润，按公告日期（防未来函数）
fields = [
    'CAPITALSTRUCTURE.total_capital',
    'ASHAREINCOME.net_profit_incl_min_int_inc',
]
data = get_financial_data(fields, ['000001.SZ', '600519.SH'], 'announce_time', start_time='20200101', end_time='20241231')
```

### 使用价值因子时的注意点

1. **未来函数**：回测务必使用 `report_type='announce_time'`，避免按报告期取数导致用到未披露数据。
2. **数据质量**：PERSHAREINDEX 等表中部分字段（如部分收益、增长率）可能返回 0 或缺失，需在策略中做过滤、填充或降权。
3. **性能与稳定性**：大量股票或长区间拉取时可能较慢或偶发卡顿；若获取不到数据，需检查字段名、权限及本地财务数据是否已下载。
4. **与行情配合**：价值因子常与 `get_market_data_ex` 的行情数据结合使用（如 PE = 市值/净利润，需 end_time 与回测当前 bar 一致，避免用「最新」导致前视）。
